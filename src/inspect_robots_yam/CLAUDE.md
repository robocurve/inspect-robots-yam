...
| `embodiment.py` | `YAMEmbodiment` — i2rt driver with joint and EEF control. Clamp backstop, optional delta→abs, lazy kinematics, default-on collision-guardrail contribution with explicit skip warnings, gripper de-norm, `SELF_PACED` pacing, operator-keypress episode end (`operator_end`), and joint-space homing/parking. Hardware seams are injected/pragma'd. |
