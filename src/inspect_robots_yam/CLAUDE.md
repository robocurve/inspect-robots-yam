...
| `config.py` | `YamConfig` / `ActServerConfig` (with the `MolmoActConfig` alias; frozen, `from_kwargs` for CLI scalars), including the opt-in `report_joint_eff` runtime-state flag and `eef_orientation` default-pin widening, plus joint and EEF action/observation-space builders. |
