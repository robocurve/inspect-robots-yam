...
- Both are plain strings: no `_FLOAT_TUPLE_FIELDS` change, `from_kwargs`
  passes them through.
...
resolved vector; a retried reset does not re-read the file.
...
- `capture <name> [--notes TEXT] [--force] [--clamp] [--park]`
  ...
  7. Safe exit: default prompts "support both arms, then press Enter to
     release torque" and closes (`close_robot_safely` path). `--park`
     instead ramps to `cfg.rest_pose` (same interpolation semantics as the
     embodiment ramp: `rest_secs`, `control_hz`, clamp + denorm each
     waypoint) and then closes. `--park` with `rest_pose=None` is a usage
     error (exit 2).
- `goto <name> [--park]`
  ...
  2. Connect, stand-clear Enter gate (same wording convention as the
     ...
     pose, then hold and prompt: Enter releases torque after a
     support-the-arms warning; `--park` ramps back to `rest_pose` first.
...
- `tests/test_poses.py`: round-trip save/load; overwrite refusal + `--force`
  ...
  list ordering + corrupt-file error naming the file; delete/rename incl.
  missing-source and existing-target errors.
- `tests/test_config.py` additions: mutual exclusion, eef rejection,
  bad-name rejection, defaults land in `from_kwargs`.
...
- `tests/test_pose_cli.py`: fake driver + scripted `OperatorIO`; capture
  ...
  list/show/delete/rename output; `-E`/config/env handling mirroring the
  health CLI tests; every usage error exits 2.
- `tests/test_api_snapshot.py`: update for new public names.

## Out of scope (say no)

- eef-mode start poses (joint-space only; config rejects the pairing).
- Per-pose collision validation beyond joint limits (the ramp already runs
  under the collision guardrail when enabled; nothing changes there).
