...
- `YamConfig` (frozen): `left_channel="can0"`, `right_channel="can1"`,
  ...
  (Superseded by plan 0004: wire 1 maps to open, with defaults
  `gripper_open=1.0`, `gripper_closed=0.0`.)
...
3. **Gripper calibration.** `gripper_open`/`gripper_closed` map MolmoAct2's gripper
   ...
   Superseded by plan 0004, which defines wire 1 as open and wire 0 as closed.
